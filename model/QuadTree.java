...
public class QuadTree {
	...
	public QuadTree(Rectangle boundary, int capacity, String name) {
	...
	public void subdivide() {
		if (this.isSubdivided == false) {
			this.northEast = new QuadTree(new Rectangle((int)(this.boundary.width/2 + this.boundary.getX()), (int)this.boundary.getY(), (this.boundary.width/2), (this.boundary.height/2)), this.capacity, "northEast");
			this.southEast = new QuadTree(new Rectangle((int)(this.boundary.width/2 + this.boundary.getX()), (int)(this.boundary.height/2 + this.boundary.getY()), (this.boundary.width/2), (this.boundary.height/2)), this.capacity, "southEast");
			this.northWest = new QuadTree(new Rectangle((int)this.boundary.getX(), (int)this.boundary.getY(), (this.boundary.width /2), (this.boundary.height/2)), this.capacity, "northWest");
			this.southWest = new QuadTree(new Rectangle((int)this.boundary.getX(), (int)(this.boundary.height/2 + this.boundary.getY()), (this.boundary.width /2), (this.boundary.height/2)), this.capacity, "southWest");
	...
	public QuadTree removeCell(int x, int y) {
		if (!this.contains(x, y)) {
			return this;
		}
		QuadTree newTree = new QuadTree(this.boundary, this.capacity, "newTree");
	...
	public QuadTree removeCell(CopyOnWriteArrayList<Cellule> cellToRemove) {
	...
	public static void main(String[] args) {
		QuadTree tree = new QuadTree(new Rectangle(0, 0, 100, 100), 2, "root");
